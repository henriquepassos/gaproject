...
crossover = uicontrol(ph,'Style','popupmenu', 'String',{'xalt_edges','scxover','orderxover'}, 'Value',1,'Position',[20 50 130 20],'Callback',@crossover_Callback);
mutation = uicontrol(ph,'Style','popupmenu', 'String',{'inversion','insertion','inversioninsertion'}, 'Value',1,'Position',[160 50 130 20],'Callback',@mutation_Callback);
...
%selectslidertxt = uicontrol(ph,'Style','text','String','Tournament size','Position',[0 20 130 20]);
%selectslider = uicontrol(ph,'Style','slider','Max',NIND,'Min',0,'Value',5,'Sliderstep',[0.01 0.05],'Position',[130 20 150 20],'Callback',@selectslider_Callback);
%selectsliderv = uicontrol(ph,'Style','text','String',5,'Position',[280 20 50 20]);
inputbutton = uicontrol(ph,'Style','pushbutton','String','Input','Position',[55 0 70 30],'Callback',@inputbutton_Callback);
runbutton = uicontrol(ph,'Style','pushbutton','String','START','Position',[0 0 50 30],'Callback',@runbutton_Callback);
...
set(fh,'Visible','on');
    ...
    function selectslider_Callback(hObject,eventdata)
        fslider_value = get(hObject,'Value');
        slider_value = round(fslider_value);
        set(hObject,'Value',slider_value);
        set(selectsliderv,'String',slider_value);
        OPT = round(slider_value)/100;
    end
    ...
    function runbutton_Callback(hObject,eventdata)
        ...
        %xovercomparisson(x, y, NIND, MAXGEN, NVAR, ELITIST, STOP_PERCENTAGE, PR_CROSS, PR_MUT, OPT, CROSSOVER, MUTATION, SELECTION, LOCALLOOP, ah1, ah2, ah3);
        script(datasets, NIND, MAXGEN, NVAR, ELITIST, STOP_PERCENTAGE, PR_CROSS, PR_MUT, OPT, CROSSOVER, MUTATION, SELECTION, LOCALLOOP, ah1, ah2, ah3);
        %run(20,x, y, NIND, MAXGEN, NVAR, ELITIST, STOP_PERCENTAGE, PR_CROSS, PR_MUT, OPT, CROSSOVER, MUTATION, SELECTION, LOCALLOOP, ah1, ah2, ah3);
        %run_ga(x, y, NIND, MAXGEN, NVAR, ELITIST, STOP_PERCENTAGE, PR_CROSS, PR_MUT, OPT, CROSSOVER, MUTATION, SELECTION, LOCALLOOP, ah1, ah2, ah3);
        end_run();
    end
